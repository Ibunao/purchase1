...
<form action="" method="POST">
    <input type="hidden" name="r" value="order/product/<?php echo $action; ?>">
...
<?php if($action == 'update') {
    ...
    <div class="form-group">
        ...
        <select name="param[purchase]" class="sec"  id="purchase" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php foreach($selectFilter['purchase'] as $v){ ?>
                <option value="<?php echo $v['purchase_id'] ?>" <?php if( isset($param['purchase_id']) && $param['purchase_id'] == $v['purchase_id'] ) { echo "selected"; } ?>><?php echo $v['purchase_name']; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="param[brand]" class="sec"  id="brand" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php foreach($selectFilter['brand'] as $v){ ?>
                <option value="<?php echo $v['brand_id'] ?>" <?php if( isset($param['brand_id']) && $param['brand_id'] == $v['brand_id'] ) { echo "selected"; } ?>><?php echo $v['brand_name']; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" name="param[name]" id="name" class="inc" value="<?php echo !empty($param['name'])?$param['name']:''; ?>" maxlength="22">(品名不多于22个字)
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" name="param[costPrice]" class="md" id="costPrice" <?php if($action=='change'||$action=='copy'){echo "readonly";} ?> value="<?php echo !empty($param['cost_price'])?$param['cost_price']:''; ?>" onkeyup="value=value.replace(/[^\d.]/g,'')">
    </div>
    ...
    <div class="form-group">
        ...
        <select name="param[scheme]" class="sec" <?php if( $action == "update" ){ echo "disabled"; } ?> id="scheme">
            ...
            <?php foreach($selectFilter['scheme'] as $v){ ?>
                <option value="<?php echo $v['scheme_id'] ?>" <?php if($action == 'update') { ?> <?php if( isset($param['scheme_id']) && $param['scheme_id'] == $v['scheme_id'] ) { echo "selected"; } ?> <?php } ?>><?php echo $v['scheme_name']; ?></option>
            <?php } ?>
        </select>
    </div>

    <?php if($action == 'update') { ?>
        <input type="hidden" id="hcolor" name="param[color_id]" value="<?php echo $param['color_id']; ?>">
        <input type="hidden" id="hscheme" name="param[scheme_id]" value="<?php echo $param['scheme_id']; ?>">
    <?php } ?>
    ...
    <div class="form-group">
        ...
        <select name="param[color]" class="width-10 chosen-select" <?php if( $action == "update" ){ echo "disabled"; } ?> data-placeholder="选择色号" id="form-field-select-3">
            ...
            <?php foreach($selectFilter['color'] as $v){ ?>
                <option value="<?php echo $v['color_id'] ?>" <?php if( isset($param['color_id']) && $param['color_id']==$v['color_id'] ) { echo "selected"; } ?>><?php echo $v['color_name']; ?></option>
            <?php }  ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="param[sizeGroup]" class="sec" id="sizeGroup" <?php if($action == 'update'||$action=='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            foreach($selectFilter['sizeGroup'] as $v){ ?>
                <option value="<?php echo $v['group_id'] ?>" <?php if( isset($param['sizeGroup']) &&  $param['sizeGroup']== $v['group_id']){ echo "selected"; } ?>><?php echo $v['size_group_name']; ?></option>
            <?php } } ?>
        </select>
    </div>
        ...
        <div class="col-xs-12">
            <div id="size" class="col-xs-4 margin-l">
                ...
                ?>
                        <div class="checkbox">
                            <label>
                                <input name="param[size][]" value="<?php echo $v['size_id'] ?>" type="checkbox" class="ace" <?php if( isset($param['size_id']) && isset($param['size'])){ foreach($param['size'] as $val){ if($val ==$v['size_id']){ echo "checked"; } } }?>>
                                <span class="lbl"><?php echo $v['size_name']; ?></span>
                            </label>
                        </div>
                <?php } } ?>
            </div>
    ...
    <div class="form-group">
        ...
        <select name="param[catBig]" class="sec" id="catBig" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php foreach($selectFilter['catBig'] as $v){ ?>
                <option value="<?php echo $v['big_id'] ?>" <?php if( isset($param['cat_b']) && $param['cat_b'] == $v['big_id'] ) { echo "selected"; } ?>><?php echo $v['cat_name']; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="param[catMiddle]" class="sec" id="catMiddle" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            foreach($selectFilter['catMiddle'] as $v){ ?>
                <option value="<?php echo $v['middle_id'] ?>" <?php if( isset($param['cat_m']) && $param['cat_m'] == $v['middle_id'] ) { echo "selected"; } ?>><?php echo $v['cat_name']; ?></option>
            <?php } } ?>
        </select>
    </div>
        ...
        <select name="param[catSmall]" class="sec" id="catSmall" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php  if(!empty($selectFilter['catSmall'])){
                foreach($selectFilter['catSmall'] as $v){ ?>
                    <option value="<?php echo $v['small_id'] ?>" <?php if( isset($param['cat_s']) && $param['cat_s'] == $v['small_id'] ) { echo "selected"; } ?>><?php echo $v['cat_name']; ?></option>
                <?php } } ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="param[season]" class="sec" id="season" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php if(!empty($selectFilter['season'])) {
                ...
                <option value="<?php echo $v['season_id']; ?>" <?php if( isset($param['season_id']) && $param['season_id'] == $v['season_id'] ) { echo "selected"; } ?>><?php echo $v['season_name']; ?></option>
            <?php }} ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="param[level]" class="sec" id="level" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php foreach($selectFilter['level'] as $v){ ?>
                <option value="<?php echo $v['level_id']; ?>" <?php if( isset($param['level_id']) && $param['level_id'] == $v['level_id']) { echo "selected"; } ?>><?php echo $v['level_name']; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group col-xs-12">
        ...
        <select name="param[wave]" class="sec" id="wave" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            ...
            <?php foreach($selectFilter['wave'] as $v){ ?>
                <option value="<?php echo $v['wave_id']; ?>" <?php if( isset($param['wave_id']) && $param['wave_id'] == $v['wave_id'] ) { echo "selected"; } ?>><?php echo $v['wave_name']; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group col-xs-12">
        ...
        <select name="param[type]" class="sec" id="type" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
            <?php foreach($selectFilter['type'] as $v){ ?>
                <option value="<?php echo $v['type_id']; ?>" <?php if( isset($param['type_id']) && $param['type_id'] == $v['type_id'] ) { echo "selected"; } ?>><?php echo $v['type_name']; ?></option>
            <?php } ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" class="inc" id="banner_url" name="param[image]" value="<?php echo !empty($param['img_url'])?$param['img_url']:''; ?>" placeholder="图片URL">
    </div>
    ...
    <div class="form-group col-xs-12" <?php if($action =='change'||$action=='copy'){echo "disabled";} ?>>
        ...
        <textarea class="form-control py inc" rows="3" id="memo" name="param[memo]"><?php echo !empty($param['memo'])?$param['memo']:''; ?></textarea>
    </div>

    <div class="form-group">
        <label for="name">状态</label>
        <select name="param[status]" class="sec">
            <option value="0" <?php echo isset($param['is_down'])&&$param['is_down']==0?'selected':''; ?>>上架</option>
            <option value="1" <?php echo isset($param['is_down'])&&$param['is_down']==1?'selected':''; ?>>下架</option>
        </select>
    </div>
